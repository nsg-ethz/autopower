            ...
            if allRecentMsmts.empty:
                ...
                if True:  # add the split measurements
                    def getMsmtVals(msmt_id):
                        msmtDf = pd.read_sql_query(sql="EXECUTE getMmtPtsBinned (%(bintime)s, %(srvmsmtid)s, %(startts)s,%(startts)s, %(endts)s)", params={"bintime": binTime, "srvmsmtid": msmt_id, "startts": startDate, "endts": endDate}, con=pgcon)
                ...
                # daily plot
                dailyMsmts = pd.read_sql_query(sql="SELECT measurement_value, measurement_dow, measurement_time, measurement_timestamp FROM getMsmtsByDow (%(mmtids)s, %(bintime)s, %(startts)s, %(endts)s)", params={"bintime": binTime, "mmtids": msmt_ids, "startts": startDate, "endts": endDate}, con=pgcon)
                ...
                return dcc.Graph(figure=plt), allRecentMsmts.to_dict('records'), False, 0, dcc.Graph(figure=hourlyPlot), dcc.Graph(figure=dailyPlot), {"component_name": "dutPowergraph", "is_loading": False}  # 0 is for n clicks reset
    else:
